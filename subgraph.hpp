...
typedef typename Traits::traversal_category        traversal_category;
    ...
    vertex_descriptor local_to_global(vertex_descriptor u_local) const {
		if ( is_root() ) return u_local;
		vertex_descriptor u_global; bool in_subgraph;
		tie( u_global , in_subgraph ) = this->find_local_vertex(u_local);
    ...
    find_local_vertex(vertex_descriptor u_local) const {
        if (is_root()) return std::make_pair(u_local, true);
        typename std::map<vertex_descriptor, vertex_descriptor>::const_iterator
            i = m_global_vertex.find(u_local);
        bool valid = i != m_global_vertex.end();
...
public: // Probably shouldn't be public....
    ...
    GlobalVertexMap m_global_vertex;
